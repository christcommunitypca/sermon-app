    ...
    <div className={`bg-white border rounded-2xl mb-4 ${isArchived ? 'border-stone-200 opacity-80' : 'border-slate-200'}`}>
      ...
      <div className="flex items-center gap-3 px-4 py-3">
        ...
        <div className="flex items-center gap-2 shrink-0 text-xs text-slate-400">
          ...
          {estimatedDuration && <span>{estimatedDuration}m</span>}
      ...
      {/* ── Expanded detail — shows fields not visible in compact row ──────── */}
      {expanded && (
        <div className="px-4 pb-4 pt-3 border-t border-slate-100 space-y-2">
          {/* Fields hidden on small screens */}
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500 sm:hidden">
            <span>{type.replace(/_/g, ' ')}</span>
            {scriptureRef && <span>{scriptureRef}</span>}
          </div>
          {dateStr && <div className="text-xs text-slate-500 md:hidden">{dateStr}</div>}
          {/* Additional fields never shown in compact row */}
          {notes && (
