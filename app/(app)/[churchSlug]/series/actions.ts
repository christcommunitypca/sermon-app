...
// All subsequent weeks are pushed out by one.
...
export async function insertLessonAfterWeekAction(
  ...
  // Push all later weeks out by 1 (descending to avoid unique constraint conflicts)
  const { data: laterWeeks } = await supabaseAdmin
    .from('series_sessions')
    .select('id, week_number')
    .eq('series_id', seriesId)
...
// Deletes a series_session row and renumbers all subsequent weeks down by 1.
// Only allowed on planned weeks with no linked teaching session.
...
export async function deleteSeriesWeekAction(
  ...
  if (ss.session_id) return { error: 'Cannot delete a week that has a linked teaching session. Delete the session first.' }
  ...
  // Renumber subsequent weeks down by 1 (ascending this time — no conflicts going down)
  const { data: laterWeeks } = await supabaseAdmin
    .from('series_sessions')
    .select('id, week_number')
    .eq('series_id', seriesId)
