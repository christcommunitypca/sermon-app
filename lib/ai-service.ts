...
// Like generateVerseInsights but takes a section (multiple verses) as one unit.
// The section label is used as the verse_ref key so it stores cleanly alongside
// per-verse data in the same verse_insights table.
...
export async function generatePericopeInsights(
  ...
  // Treat the whole section as a single "verse" entry for the prompt
  const combinedVerseData: import('@/lib/esv').VerseData = {
    verse_ref: input.section.label,
    verse_num: 1,
  ...
  const sectionInput: VerseInsightInput = {
    ...
    pastorNotes:  input.pastorNotes?.length
      ? { [input.section.label]: input.pastorNotes }
      : {},
    selectedWords: {},
  }

  // Run both category batches in parallel (same as short VBV path)
  const batchResults = await Promise.allSettled(
    VerseInsightsPrompt.CATEGORY_BATCHES.map(async (categories) => {
      ...
      type RawBatchItem = { verse_ref?: string; category?: string; items?: { title?: string; content?: string }[] }
      if (!Array.isArray(completion.parsed)) {
        throw new AIError('malformed_response', 'Expected JSON array')
      }

      const rows: RawVerseInsight[] = (completion.parsed as RawBatchItem[])
        .filter(item => item && typeof item.verse_ref === 'string' && typeof item.category === 'string' && Array.isArray(item.items))
        .map(item => ({
          verse_ref: input.section.label,   // always key by section label
          category:  item.category as string,
          items:     (item.items ?? []).slice(0, 4).map(i => ({ title: i.title ?? '', content: i.content ?? '' })),
        }))
